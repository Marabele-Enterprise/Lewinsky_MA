...
function xlsworker(data, cb) {
	var worker = new Worker('/public/js/dropUploader/xlsworker.js');
...
function addProducts(output){
	$('#loadingImg').fadeIn();
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "/product/addGeoData",
		data: {productsData:output},
		...
		error: function(e){
			console.log(e);
			$('#loadingImg').fadeOut();
			$("#feedback").append("<div class='alert alert-success alert-dismissable'> <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Products uploaded successfuly</div>");
		}
	}).done(function(data) {
		console.log(data);
		$('#loadingImg').fadeOut();
